...
Retry imediato é a forma mais simples de reexecutar uma operação logo em seguida, sem esperar um intervalo. É útil quando a falha é muito breve e a latência adicional é pequena, porém pode sobrecarregar o sistema se muitas requisições falharem ao mesmo tempo.

#### Retry com Delay

Retry com delay adiciona um intervalo entre as tentativas, geralmente usando backoff exponencial. Esse tipo é mais seguro do que o retry imediato porque reduz a pressão sobre o serviço e aumenta a chance de a dependência recuperar.

#### Retry Budget

Retry budget define um limite de retries com base no custo total de tentativas. A ideia é evitar que o sistema gaste recursos demais tentando repetir operações que provavelmente continuarão falhando. É uma estratégia importante para controlar sobrecarga e manter a estabilidade.

> Importante, o Retry é a literalmente a tentativa, o backoff pergunta quando tempo eu vou demorar para chamar novamente, e o exponential, é o tempo progressivo que vou aplicando pra cada tentativa.s

#### Retry por HTTP Status

Esse tipo de retry é aplicado quando a falha vem de respostas HTTP específicas, como 408, 429, 500, 502, 503 ou 504. Muitas vezes, esses status indicam problemas temporários do servidor ou saturação, então repetir a requisição pode resolver o problema.

#### Retry por Exception

Retry por exception é usado quando a operação lança exceções específicas, normalmente relacionadas a timeout, conexão abortada, conexão recusada ou falhas transitórias. Nesse caso, o sistema decide tentar novamente apenas para tipos de erro que fazem sentido serem tratados como temporários.

#### Retry Idempotente

Retry idempotente é o tipo mais seguro em ambientes distribuídos. Ele pressupõe que a operação pode ser repetida sem causar efeitos colaterais indesejados, como reprocessar um pagamento ou criar dois registros iguais. Esse tipo de retry é essencial quando a operação pode ser executada mais de uma vez sem quebrar a consistência.
